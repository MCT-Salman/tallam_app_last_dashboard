...
// import { ChevronDown, ChevronUp, Loader2, Users, GraduationCap, BookOpen, Layers, Eye } from "lucide-react"
// import { 
//   getInstructors,
//   getSpecializations,
//   getCourses,
...
//             المشتركين النشطين لكل مستوى
//           </CardTitle>
//           <p className="text-xs text-muted-foreground">فلترة وعرض تفصيلي للمشتركين حسب المستوى</p>
//         </CardHeader>
//         <CardContent className="space-y-4">
//           {/* Filters */}
//           <div className="grid grid-cols-1 md:grid-cols-5 gap-3 p-3 rounded-lg border bg-gradient-to-br from-muted/40 to-muted/10">
...
//             <div className="space-y-1">
//               <Label>بحث</Label>
//               <Input value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1) }} placeholder="بحث بالاسم/الهاتف/المستوى..." />
//             </div>

//             <div className="space-y-1">
//               <Label>حد أدنى للمشتركين</Label>
//               <Input type="number" value={minSubscribers} onChange={(e) => { setMinSubscribers(e.target.value); setCurrentPage(1) }} />
//             </div>
//           </div>

//           {/* Table (md+) */}
//           <div className="rounded-md border overflow-x-auto hidden md:block shadow-sm">
//             <Table className="min-w-full">
//               <TableHeader>
//                 <TableRow>
//                   <TableHead className="cursor-pointer" onClick={() => handleSort('instructor')}>
//                     <div className="flex items-center gap-1">المدرس {sortBy==='instructor' ? (sortOrder==='asc'?'↑':'↓') : ''}</div>
//                   </TableHead>
//                   <TableHead className="cursor-pointer" onClick={() => handleSort('specialization')}>
//                     <div className="flex items-center gap-1">الاختصاص {sortBy==='specialization' ? (sortOrder==='asc'?'↑':'↓') : ''}</div>
//                   </TableHead>
//                   <TableHead className="cursor-pointer" onClick={() => handleSort('course')}>
//                     <div className="flex items-center gap-1">الدورة {sortBy==='course' ? (sortOrder==='asc'?'↑':'↓') : ''}</div>
//                   </TableHead>
//                   <TableHead className="cursor-pointer" onClick={() => handleSort('level')}>
//                     <div className="flex items-center gap-1">المستوى {sortBy==='level' ? (sortOrder==='asc'?'↑':'↓') : ''}</div>
//                   </TableHead>
//                   <TableHead className="cursor-pointer text-right" onClick={() => handleSort('count')}>
//                     <div className="flex items-center gap-1 justify-end">عدد المشتركين {sortBy==='count' ? (sortOrder==='asc'?'↑':'↓') : ''}</div>
//                   </TableHead>
//                 </TableRow>
//               </TableHeader>
//               <TableBody>
...
//                     <TableCell className="whitespace-nowrap">{row.instructorName}</TableCell>
//                     <TableCell className="whitespace-nowrap">{row.specializationName}</TableCell>
//                     <TableCell className="whitespace-nowrap">{row.courseTitle}</TableCell>
//                     <TableCell className="whitespace-nowrap">{row.levelName}</TableCell>
//                     <TableCell className="text-right">
//                       <div className="flex items-center justify-end gap-2">
//                         <Badge variant="secondary">{row.count} مشترك</Badge>
//                         {row.count > 0 && (
//                           <Button size="icon" variant="ghost" title="التفاصيل" onClick={() => { setDetailRow(row); setDetailSearch(''); setDetailPage(1); setDetailOpen(true) }} className="h-8 w-8">
...
//                 )) : (
//                   <TableRow>
//                     <TableCell colSpan={5} className="text-center py-6 text-muted-foreground">لا توجد بيانات مطابقة</TableCell>
//                   </TableRow>
//                 )}
//               </TableBody>
//             </Table>
...
//             {pageRows.length > 0 ? pageRows.map(row => (
//               <Card key={`card-${row.levelId}`} className="border-none shadow-lg hover:shadow-xl transition-shadow bg-gradient-to-br from-blue-50/50 to-indigo-50/50">
...
//                     <span className="flex items-center gap-1"><GraduationCap className="w-3 h-3" /> {row.instructorName}</span>
//                     <span>•</span>
//                     <span className="flex items-center gap-1"><Layers className="w-3 h-3" /> {row.specializationName}</span>
//                     <span>•</span>
//                     <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" /> {row.courseTitle}</span>
//                   </div>
//                 </CardHeader>
//                 <CardContent className="pt-0">
//                   <div className="flex items-center justify-between">
//                     <Badge variant="secondary" className="text-sm">{row.count} مشترك</Badge>
//                     {row.count > 0 && (
//                       <Button size="icon" variant="ghost" title="التفاصيل" onClick={() => { setDetailRow(row); setDetailSearch(''); setDetailPage(1); setDetailOpen(true) }} className="h-8 w-8">
//                         <Eye className="w-4 h-4" />
//                       </Button>
...
//               </Card>
//             )) : (
//               <div className="text-center py-6 text-muted-foreground">لا توجد بيانات مطابقة</div>
//             )}
//           </div>

//           {/* Pagination */}
//           <div className="flex items-center justify-between text-sm">
//             <div>عرض {(pageStart+1)} - {Math.min(pageStart + itemsPerPage, totalItems)} من {totalItems}</div>
//             <div className="flex items-center gap-2">
//               <Select value={itemsPerPage.toString()} onValueChange={(v) => { setItemsPerPage(Number(v)); setCurrentPage(1) }}>
//                 <SelectTrigger className="w-[110px]"><SelectValue placeholder="عدد الصفوف" /></SelectTrigger>
//                 <SelectContent>
...
//       {/* Dialog: Subscribers list */}
//       <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
//         <DialogContent className="sm:max-w-3xl max-h-[95vh] overflow-y-auto p-0">
//           <div className="border-b bg-white/60 backdrop-blur p-4">
//             <DialogTitle className="text-right">
//               {detailRow ? (
//                 <div className="space-y-1">
//                   <div className="text-lg font-bold">{detailRow.levelName}</div>
//                   <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
//                     <span className="flex items-center gap-1"><GraduationCap className="w-3 h-3" /> {detailRow.instructorName}</span>
//                     <span>•</span>
//                     <span className="flex items-center gap-1"><Layers className="w-3 h-3" /> {detailRow.specializationName}</span>
//                     <span>•</span>
//                     <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" /> {detailRow.courseTitle}</span>
//                     <Badge variant="secondary" className="ml-auto">{detailRow.count} مشترك</Badge>
//                   </div>
//                 </div>
//               ) : 'تفاصيل المستوى'}
//             </DialogTitle>
...
//             <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-3 rounded-lg border bg-gradient-to-br from-muted/40 to-muted/10">
//               <div className="space-y-1">
//                 <Label>بحث داخل المشتركين</Label>
//                 <Input value={detailSearch} onChange={(e) => { setDetailSearch(e.target.value); setDetailPage(1) }} placeholder="بحث بالاسم/الهاتف/الكود" />
//               </div>
//               <div className="space-y-1">
//                 <Label>عدد الصفوف</Label>
//                 <Select value={detailPerPage.toString()} onValueChange={(v) => { setDetailPerPage(Number(v)); setDetailPage(1) }}>
//                   <SelectTrigger><SelectValue placeholder="عدد" /></SelectTrigger>
//                   <SelectContent>
//                     <SelectItem value="10">10</SelectItem>
//                     <SelectItem value="20">20</SelectItem>
//                     <SelectItem value="50">50</SelectItem>
...
//                       <TableCell className="text-right" dir="ltr">{s.phone}</TableCell>
//                       <TableCell className="text-right font-mono">{s.code}</TableCell>
//                     </TableRow>
//                   )) : (
//                     <TableRow>
//                       <TableCell colSpan={3} className="text-center py-6 text-muted-foreground">لا توجد نتائج</TableCell>
//                     </TableRow>
//                   )}
//                 </TableBody>
//               </Table>
...
//             <div className="flex items-center justify-between text-sm">
//               <div>عرض {(detailStart+1)} - {Math.min(detailStart + detailPerPage, detailTotal)} من {detailTotal}</div>
//               <div className="flex items-center gap-1">
//                 <Button variant="outline" size="sm" onClick={() => setDetailPage(p => Math.max(1, p-1))} disabled={detailPage===1}>السابق</Button>
//                 <span className="px-2">{detailPage}/{detailTotalPages}</span>
//                 <Button variant="outline" size="sm" onClick={() => setDetailPage(p => Math.min(detailTotalPages, p+1))} disabled={detailPage===detailTotalPages}>التالي</Button>
//               </div>
//             </div>
//           </div>
//         </DialogContent>
